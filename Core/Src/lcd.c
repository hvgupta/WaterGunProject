...
void LCD_DrawEllipse ( uint16_t usC, uint16_t usP, uint16_t SR, uint16_t LR, uint16_t usColour)
{	
	/*
	 *  Task 3 : Implement LCD_DrawEllipse by using LCD_DrawDot
	 *
	 */
	const float SRsqrd = SR*SR;
	const float LRsqrd = LR*LR;
	const float RHS = SRsqrd*LRsqrd;
	for (int xdiff = 0; xdiff < SR; xdiff++){
		int ydiff = sqrt2((RHS-LRsqrd*xdiff*xdiff)/SRsqrd);
		LCD_DrawDot(usC-xdiff,usP-ydiff,usColour);
		LCD_DrawDot(usC-xdiff,usP+ydiff,usColour);
		LCD_DrawDot(usC+xdiff,usP-ydiff,usColour);
		LCD_DrawDot(usC+xdiff,usP+ydiff,usColour);
	}
	for (int ydiff = 0; ydiff < LR; ydiff++){
		int xdiff = sqrt2((RHS-SRsqrd*ydiff*ydiff)/LRsqrd);
		LCD_DrawDot(usC-xdiff,usP-ydiff,usColour);
		LCD_DrawDot(usC-xdiff,usP+ydiff,usColour);
		LCD_DrawDot(usC+xdiff,usP-ydiff,usColour);
		LCD_DrawDot(usC+xdiff,usP+ydiff,usColour);
