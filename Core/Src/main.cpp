...
static LED::LED ledlights{20};
static reloadingProcess::Reload Reloadobj{100,100};
static shootingProcess::singleShot singleShotobj{0, 1000, 500, 1200};
static shootingProcess::continousShots continousShotsobj{0, 700, 500, 1200};
/* USER CODE END PD */
...
void EXTI0_IRQHandler(void)
{
  /* USER CODE BEGIN EXTI0_IRQn 0 */
	...
	if (__HAL_GPIO_EXTI_GET_IT(GPIO_PIN_0) != RESET)
	{
		...
		WaterGun::STATUS curStatus = GunState.status;

		if (curStatus == WaterGun::STATUS::OFF_STATE){
			GunState.status = WaterGun::STATUS::RELOAD_STATE;	//Status changes by interrupt using K1
		}
		else if (curStatus == WaterGun::STATUS::RELOAD_STATE){
			GunState.status = WaterGun::STATUS::SINGLE_SHOOT_STATE;
		  }
		else if (curStatus == WaterGun::STATUS::SINGLE_SHOOT_STATE){
			GunState.status = WaterGun::STATUS::CONTINIOUS_SHOOT_STATE;
		}
		else{								//CONTINIOUS_SHOOT_STATE
			GunState.status = WaterGun::STATUS::OFF_STATE;
		}
		/*Key1 code END*/
...
// void ledlights[i].setRGBValue (int LEDidx, int Red, int Green, int Blue){	//0-255 scale
...
void brightnessAndTransmit(const int brightness, const HAL_Ticks delay){
    ledlights.Set_Brightness(brightness);
...
int main(void)
{
    ...
    //   if (htim->Channel != HAL_TIM_ACTIVE_CHANNEL_2){return;}  // if the interrupt source is channel1

    //   if (Is_First_Captured==0) // if the first value is not captured
    //   {
    //       IC_Val1 = HAL_TIM_ReadCapturedValue(&htim1, TIM_CHANNEL_2); // read the first value
    ...
    while (1)
    {
        ...
        if (curStatus == WaterGun::STATUS::OFF_STATE)
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                ledlights[i].setRGBValue(255 - 2 * i, 255 - 2 * i, 4 * i); // Yellow to less yellow
            }
            for (int i = 0; i < ledlights.getSize(); i++){brightnessAndTransmit(3*i,25);}
            for (int i = ledlights.getSize()-1; i >= 0; i--){brightnessAndTransmit(3*i,25);}
        }
        else if (curStatus == WaterGun::STATUS::RELOAD_STATE)
        {
            ...
            if (Reloadobj.getTriggerState() == true)
            {
                ...
                for (int i = 0; i < 10 + 3; i++)
                {
                    if (i < 10)
                    {
                        ...
                        ledlights[9 - i].setRGBValue(42, 254, 183); // Cyan
                        // Turn on head of middleright LED
                        ledlights[10 + i].setRGBValue(42, 254, 183); // Cyan
                    }
                    if ((i - 3) >= 0)
                    {
                        // Turn on tail of middleleft LED
                        ledlights[9 - (i - 3)].setRGBValue( 0, 0, 0);
                        // Turn off tail of middleright LED
                        ledlights[10 + (i - 3)].setRGBValue(0, 0, 0);
                    }
                    brightnessAndTransmit(60,77);
                }
            ...
            else
            {
                ...
                for (int i = 0; i < 20; i++)
                {
                    ledlights[i].setRGBValue(42, 254, 183); // Cyan
                }
                for (int i = 0; i < 20; i++){brightnessAndTransmit(3*i,25);}
                for (int i = 19; i >= 0; i--){brightnessAndTransmit(3*i,25);}
            }
        }
        else if (curStatus == WaterGun::STATUS::SINGLE_SHOOT_STATE)
        {
            ...
            if (singleShotobj.getTriggerState() == true)
            {
                ...
                for (int i = 0; i < 10 + 3; i++)
                {
                    if (i < 10)
                    {
                        ...
                        ledlights[i].setRGBValue(138, 43, 226); // Blue-purple
                        // Turn on head of rightmost LED
                        ledlights[19 - i].setRGBValue(138, 43, 226); // Blue-purple
                    }
                    if ((i - 3) >= 0)
                    {
                        // Turn on tail of leftmost LED
                        ledlights[i - 3].setRGBValue(0, 0, 0);
                        // Turn off tail of rightmost LED
                        ledlights[19 - i + 3].setRGBValue(0, 0, 0);
                    }
                    brightnessAndTransmit(70,77);
                }
        ...
        { // CONTINIOUS_SHOOT_STATE
            ...
            if (continousShotsobj.getTriggerState() == true)
            {
                ...
                for (int i = 0; i < 10 + 3; i++)
                {
                    ...
                    {
                        // Turn on head of leftmost LED
                        ledlights[i].setRGBValue(255, 105, 100); // Pink
                        // Turn on head of rightmost LED
                        ledlights[19 - i].setRGBValue(255, 105, 100); // Pink
                    }
                    if ((i - 3) >= 0)
                    {
                        // Turn on tail of leftmost LED
